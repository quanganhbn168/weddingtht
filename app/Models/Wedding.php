...
class Wedding extends Model implements HasMedia
{
    ...
    protected $casts = [
        ...
        'groom_reception_date_2' => 'date',
        'bride_reception_date_2' => 'date',
        'groom_reception_time_2' => 'datetime',
        'bride_reception_time_2' => 'datetime',
        'content' => 'array',
        'groom_qr_info' => 'array',
        'bride_qr_info' => 'array',
        'is_active' => 'boolean',
    ...
    /**
     * Generate VietQR URL for groom's bank account.
     * Falls back to uploaded QR image if no bank info configured.
     */
    public function getGroomQrUrl(): string
    {
        $info = $this->groom_qr_info;
        if (!empty($info['bank_bin']) && !empty($info['account_number'])) {
            $bin    = $info['bank_bin'];
            $acct   = $info['account_number'];
            $name   = urlencode($info['account_name'] ?? '');
            $desc   = urlencode($info['description'] ?? '');
            return "https://img.vietqr.io/image/{$bin}-{$acct}-compact2.jpg?accountName={$name}&addInfo={$desc}";
        }
        // Fallback: uploaded QR image
        $url = $this->getMediaUrlWithDemoFallback('groom_qr');
        return $url ?: asset('images/qr-placeholder.png');
    }

    /**
     * Generate VietQR URL for bride's bank account.
     * Falls back to uploaded QR image if no bank info configured.
     */
    public function getBrideQrUrl(): string
    {
        $info = $this->bride_qr_info;
        if (!empty($info['bank_bin']) && !empty($info['account_number'])) {
            $bin    = $info['bank_bin'];
            $acct   = $info['account_number'];
            $name   = urlencode($info['account_name'] ?? '');
